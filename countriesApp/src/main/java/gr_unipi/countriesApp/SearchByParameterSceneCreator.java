...
public class SearchByParameterSceneCreator extends SceneCreator {
	...
	public SearchByParameterSceneCreator(double width, double height) {
		...
		buttonBox.getChildren().addAll(searchByCountryButton, searchByLangButton, searchByCurrencyButton, returnButton); 
	...
	private void updateSearchHistory(String query, String type) {
	    // Check if the search history already contains the item
	    boolean containsItem = searchHistoryList.stream()
	            .anyMatch(item -> item.getQuery().equals(query) && item.getType().equals(type));
	    
	    // If the item is not already in the search history, add it
	    if (!containsItem) {
	...
	private void performSearch(String query, String type) {
	    ...
	    try {
	        switch (type) {
	            case "Country":
	                List<CountriesInfo> countryResult = CountriesAPI.getCountriesAPIService().searchCountriesByName(query);
	                ...
	                break;
	            case "Language":
	                List<CountriesInfo> languageResult = CountriesAPI.getCountriesAPIService().searchCountriesByLanguage(query);
	            ...
	            case "Currency":
	                ...
	                List<CountriesInfo> allCountries = CountriesAPI.getCountriesAPIService().searchAllCountries();
		...
		try {
			...
			result.ifPresent(countryName -> {
				try {
					...
					// by country name
					List<CountriesInfo> countriesList = CountriesAPI.getCountriesAPIService().searchCountriesByName(countryName);
		...
		try {
			...
			result.ifPresent(language -> {
				try {
					// Call the appropriate method from the CountriesAPIService class for searching
					// by language
					List<CountriesInfo> countriesList = CountriesAPI.getCountriesAPIService().searchCountriesByLanguage(language);
					if (!countriesList.isEmpty()) {
						// Clear previous search results
						countriesTableView.getItems().clear();
	...
	private void searchByCurrency() {
		try {
			...
			allCurrencies.sort(Comparator.naturalOrder());
	...
	private List<String> getAllLanguages() {
		try {
			...
			List<CountriesInfo> countriesList = CountriesAPI.getCountriesAPIService().searchAllCountries();
